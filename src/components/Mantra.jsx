      ...
      <div className="container mx-auto max-w-6xl p-4 md:p-8">
        {loading ? (
          ...
          >
            {mantras.length > 0 ? (
              mantras.map((mantra) => (
                <motion.div key={mantra.id} variants={itemVariants}>
                  <Link to={`/mantra/${mantra.id}`}>
                    <Card className="h-full flex flex-col">
                      <CardImage src={mantra.imageData} alt="Mantra" />
              ...
              >
                <p className="text-xl text-primary/80 font-khand">No mantras available yet.</p>
              </motion.div>
            )}
          </motion.div>
        )}
