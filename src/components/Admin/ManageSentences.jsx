...
const ManageSentences = () => {
    ...
    const collectionRef = collection(firestore, 'sentencewords');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (currentItem) {
                const docRef = doc(firestore, 'sentencewords', currentItem.id);
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this Item?")) {
            try {
                await deleteDoc(doc(firestore, 'sentencewords', id));
                fetchItems();
            } catch (error) {
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-3xl font-khand font-bold text-primary">Manage Sentences & Words</h2>
                ...
                {items.map(item => (
                    <Card key={item.id} className="relative group">
                        <div className="h-48 overflow-hidden bg-gray-100">
                            {item.imageData ? (
                                <img src={item.imageData} alt={item.text} className="w-full h-full object-cover" />
                            ) : (
                                <div className="flex items-center justify-center h-full text-gray-400"><FaImage size={40} /></div>
                            )}
                        </div>
                        <CardContent>
                            <h3 className="text-xl font-bold text-primary font-khand mb-2 truncate">{item.text}</h3>
                            <p className="text-gray-600 text-sm font-khand line-clamp-3 mb-4">{item.explanation}</p>

                            <div className="flex space-x-2 mt-auto">
                                <Button variant="outline" className="text-sm px-3 py-1" onClick={() => openEditModal(item)}>
                                    <FaEdit />
                                </Button>
                                <Button variant="danger" className="text-sm px-3 py-1" onClick={() => handleDelete(item.id)}>
                                    <FaTrash />
                                </Button>
                            </div>
                        </CardContent>
                    ...
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                        <div className="p-6">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <div
                                    onClick={() => document.getElementById('sentenceImageInput').click()}
