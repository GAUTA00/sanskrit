      ...
      <div className="container mx-auto max-w-6xl p-4 md:p-8">
        {loading ? (
          ...
          >
            {kathas.length > 0 ? (
              kathas.map((katha) => (
                <motion.div key={katha.id} variants={itemVariants}>
                  <Link to={`/katha/${katha.id}`}>
                    <Card className="h-full flex flex-col">
                      <CardImage src={katha.imageData} alt="Katha" />
              ...
              >
                <p className="text-xl text-primary/80 font-khand">No Kathas available yet.</p>
              </motion.div>
            )}
          </motion.div>
        )}
