      ...
      <div className="container mx-auto max-w-6xl p-4 md:p-8">
        {loading ? (
          ...
          >
            {shlokas.length > 0 ? (
              shlokas.map((shlok) => (
                <motion.div key={shlok.id} variants={itemVariants}>
                  <Link to={`/shlok/${shlok.id}`}>
                    <Card className="h-full flex flex-col">
                      <CardImage src={shlok.imageData} alt="Shlok" />
              ...
              >
                <p className="text-xl text-primary/80 font-khand">No shlokas available yet.</p>
              </motion.div>
            )}
          </motion.div>
        )}
